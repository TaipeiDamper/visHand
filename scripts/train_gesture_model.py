...
def main():
    data_path = Path("scratch/gesture_calibration_dataset.json")
    ...
    # Model Save (Save all X, Y for max accuracy in production)
    models_dir = Path("core/models")
    models_dir.mkdir(parents=True, exist_ok=True)
    out_path = models_dir / "gesture_knn.npz"
    
    # We save all data (not just train split) to be the final reference dataset
    np.savez_compressed(out_path, features=X, labels=Y)
    print(f"Successfully saved 1-NN dataset model to {out_path}")
